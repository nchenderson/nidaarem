sdaarem.lasso <- function(par, X, y, lambda, stplngth=NULL, nesterov.init=FALSE, family = c("gaussian", "binomial"), 
                          sub.type="threshold", control=list()) {
  ...
  if(nesterov.init & family=="gaussian") {
    ...
    if(!neirun$convergence) {
      ans <- switch(base_fn, gauss_ngtp = sub_daarem_lasso_gaussian_ngtp(par, X, y, lambda, stplngth, nlag, a1, kappa, maxiter - nest.fpevals, tol, mon.tol, cycl.mon.tol, sub.type),
                    gauss_pgtn = sub_daarem_lasso_gaussian_pgtn(par, X, y, lambda, stplngth, nlag, a1, kappa, maxiter - nest.fpevals, tol, mon.tol, cycl.mon.tol, sub.type),
