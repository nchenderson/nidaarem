daarem_lasso_gaussian_ngtp2 <- function(par, X, y, lambda, stplngth, nlag, a1, kappa, maxiter, 
  ...
  fold <- xnew - xold
  objfn_track[1] <-  LassoObjFn_ngtp(xold, X, Xty, yty, lasso.pen)
  ...
  objfn_track[2] <-  LassoObjFn_ngtp(xnew, X, Xty, yty, lasso.pen)
  krg <- ss.resids    
  ...
  while(k < maxiter) {
    ...
    objfn_track[k + 2] <-  LassoObjFn_ngtp(xnew, X, Xty, yty, lasso.pen)
    if(count==nlag) {
      count <- 0
      if(length(mtol)==2) {
  ...
  value.obj <- LassoObjFn_ngtp(xnew, X, Xty, yty, lasso.pen)
